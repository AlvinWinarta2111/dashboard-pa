...
agg = filtered.groupby(group_field).agg(
    ...
    maintenance_delay_hours=("DELAY", lambda x: x[df.loc[x.index,"CATEGORY"]=="Maintenance"].sum()),
...
with right_col:
    st.subheader("Delay Breakdown (Donut)")
    if not donut_data.empty: st.plotly_chart(donut_fig,use_container_width=True)
    else: st.info("No delay data available for the selected filters.")
