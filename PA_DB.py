...
with tabs[0]:
    ...
    # Build the Plotly trend figure (unchanged look)
    fig_trend = go.Figure()
    fig_trend.add_trace(
        go.Bar(
            x=trend[x_field],
...
<br>
This [Python tutorial](https://www.youtube.com/watch?v=nomaHDS2LnA) explains what a `NameError` is and how to fix it by ensuring your variables are defined in the correct scope before being used.
...
/mount/src/dashboard-pa/PA_DB.py:809 in <module>


806 │ │ # -------------------------

807 │ │ # KPI calculations and KPI text generation

808 │ │ # -------------------------

❱ 809 │ │ if not filtered.empty:

810 │ │ │ total_delay = filtered["DELAY"].sum()

811 │ │ │ available_time = None
...
why is it still the same error.. can we look back first on what we did before actually proceeding



You're absolutely right to question this. My apologies; the NameError persists because of how Streamlit processes script execution. I incorrectly assumed that moving the KPI logic would solve the variable scope issue, but I was wrong. Let's look at what's happening and the correct fix.

What happened?
The error NameError: name 'filtered' is not defined occurs because Streamlit re-runs your entire script from top to bottom on every user interaction, including button clicks. The filtered variable is created after the sidebar widgets are rendered and their values are captured.

In the previous version, the code for the "Generate PDF" button was placed before the filtered variable was defined. When you clicked the button, Streamlit reran the script. The script first reached the PDF button's code block. Since st.button returned True, the code inside the if statement tried to execute if not filtered.empty, but at that exact moment in the script's execution, filtered had not yet been created, leading to the NameError.

The Correct Solution
The solution is to ensure the filtered DataFrame is always created and populated before any component attempts to use it. This means moving the data filtering logic from its current position to a higher-level, global scope in the script.

I will perform these changes:

Move the filtering logic for the filtered DataFrame to a point in the script that is executed on every run, before the PDF button and the main dashboard content.

Keep the KPI calculations and _pdf_kpi_text generation inside the if st.sidebar.button(...) block. This is the correct placement to ensure the KPI values are calculated based on the latest state of the filtered DataFrame when the button is pressed.

Maintain the 'PICA' column changes you previously requested and confirmed were working.

This new structure guarantees that the filtered variable always exists and is up to date with the user's sidebar selections before the PDF generation logic attempts to use it.

Below is the full, updated code that incorporates these fixes. You can use it to replace your current PA_DB.py file.
