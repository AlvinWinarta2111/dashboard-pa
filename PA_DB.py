# PA_DB.py
# Full updated single-file dashboard:
# - Main dashboard (Data Delay Time)
# - Reliability tab (MTTR & MTBF weekly + monthly)
# - PDF export (Option 1: attempt PNGs via Plotly/Kaleido, fallback to KPI-only PDF)
# - Global Year multi-select filter (sidebar)
# - Drilldown AgGrid auto-resize and fixed columns
# Inline comments are included to ease debugging.
...
# Attempt to import ReportLab for PDF creation (graceful degrade if not present)
try:
...
# -------------------------
# Helper: convert Plotly figure -> PNG bytes (kaleido)
# -------------------------
def _fig_to_png_bytes(fig):
    """
    Try to convert a Plotly figure to PNG bytes via fig.to_image().
    Returns bytes or None if conversion fails. Caller should handle logging/reporting.
    """
    if fig is None:
        return None
    try:
        # Using Plotly's built-in conversion (Kaleido). This may fail if Kaleido or Chrome
        # is not available in the execution environment.
        img_bytes = fig.to_image(format="png")
        if isinstance(img_bytes, (bytes, bytearray)):
            return bytes(img_bytes)
    except Exception as e:
        # Don't crash; return None so PDF generator can continue with what it has.
        # Save error in session_state for debugging if desired.
        st.session_state.setdefault("_last_fig_error", str(e))
        return None
    return None

# -------------------------
# Helper: create PDF bytes given title, KPI text and list of PNG bytes
# -------------------------
def _create_pdf_bytes(title, kpi_text, png_bytes_list):
    """
    Create a PDF in memory using ReportLab. Uses PNG bytes for charts if provided.
    ...
    # Embed each chart PNG (one after another)
    for png in png_bytes_list or []:
        if png is None:
            continue
        try:
            bio = BytesIO(png)
            rl_img = RLImage(bio, width=6.5 * inch)  # fit the page width approximately
            elements.append(Spacer(1, 0.08 * inch))
    ...
    try:
        ...
        st.session_state.setdefault("_last_pdf_error", str(e))
        return None

# -------------------------
# Auto-hide sidebar client script (kept minimal)
# -------------------------
st.markdown(
    """
    <script>
...
# -------------------------
# Config: RAW Excel URL (same as your repo)
# -------------------------
...
# -------------------------
# Load and clean 'Data Delay Time' sheet
# -------------------------
...
def load_data_from_url():
    ...
    # detect header row inside first 20 rows
    header_row = None
    for i in range(20):
    ...
    # normalize common columns (case-insensitive matching)
    replacements = {
        ...
        "MTN NOTE": "MTN_NOTE", "NOTE": "NOTE", "START": "START", "STOP": "STOP", "EQUIPMENT": "EQUIPMENT",
        "EQ. DESC.": "EQ_DESC", "CATEGORY": "CATEGORY", "PERIOD_MONTH": "PERIOD_MONTH", "DATE": "DATE"
    }
    ...
    # parse START/STOP into AVAILABLE_HOURS (if present)
    ...
    # textual columns normalization
    text_cols = ["MTN_DELAY_TYPE", "SCH_MTN", "UNSCH_MTN", "MINING_DELAY", "WEATHER_DELAY",
                 "OTHER_DELAY", "MTN_NOTE", "NOTE", "EQ_DESC", "EQUIPMENT", "CATEGORY", "PERIOD_MONTH", "DATE"]
    ...
    # SUB_CATEGORY detection (Scheduled / Unscheduled)
    ...
    # Keep data from 2023 onwards (user requested)
    if "YEAR" in df.columns:
        df = df[df["YEAR"].notna() & (df["YEAR"] >= 2023)]

    # Compose EQUIPMENT_DESC
    ...
    # WEEK_START computation (iso-week Monday) for accurate 52-week limiting
    ...
    # assign ISO-week to the month containing the week end (latest month of that week)
    try:
...
def compute_mtbf_mttr_from_url(raw_url):
    """
    Read Data Operational sheet and compute weekly/monthly MTBF and MTTR.
    MTBF_hours = total_operational_hours / count_rows
    MTTR_hours = total_maintenance_delay / count_rows
    COUNT is simple row count (as requested).
    """
    try:
    ...
    # find sheet named 'Data Operational' (flexible)
    sheet_name = None
    ...
    # parse operational hours -> decimal hours
    ...
    # YEAR / WEEK (iso) from date parsed
    def _from_date_to_iso(x):
        try:
            if pd.isna(x):
                return (None, None)
            d = pd.to_datetime(x)
    ...
    # WEEK_START and PERIOD_MONTH from week end
    try:
        # Try period-based week start (Monday)
    ...
    # compute MTBF & MTTR per weekly group
    weekly_group["MTBF_hours"] = weekly_group.apply(
        lambda r: (r["total_operational_hours"] / r["count_rows"]) if (r["count_rows"] and r["count_rows"] > 0) else float("nan"),
        axis=1
    )
    weekly_group["MTTR_hours"] = weekly_group.apply(
        lambda r: (r["total_maintenance_delay"] / r["count_rows"]) if (r["count_rows"] and r["count_rows"] > 0) else float("nan"),
        axis=1
    )

    # derive labels & week_start for ordering
    ...
    monthly_group["MTBF_hours"] = monthly_group.apply(
        lambda r: (r["total_operational_hours"] / r["count_rows"]) if (r["count_rows"] and r["count_rows"] > 0) else float("nan"),
        axis=1
    )
    monthly_group["MTTR_hours"] = monthly_group.apply(
        lambda r: (r["total_maintenance_delay"] / r["count_rows"]) if (r["count_rows"] and r["count_rows"] > 0) else float("nan"),
        axis=1
    )
    try:
...
# call the MTBF/MTTR computation
_mtbf_mttr_res = compute_mtbf_mttr_from_url(RAW_URL)

# expose global values for KPIs if needed
...
# Prepare KPI short text (will be updated after KPI calc below - we predefine)
st.session_state.setdefault("_last_pdf", None)
st.session_state.setdefault("_last_fig_error", None)
st.session_state.setdefault("_last_pdf_error", None)

# Provide a short help if ReportLab/Kaleido missing
if not REPORTLAB_AVAILABLE:
    st.sidebar.info("PDF export disabled: ReportLab not installed in this environment.")
else:
    st.sidebar.caption("PDF export will embed charts if chart -> PNG conversion succeeds.")

# -------------------------
# Time granularity (same as original)
# -------------------------
granularity = st.sidebar.selectbox("Time granularity", options=["WEEK", "PERIOD_MONTH"], index=1)

# -------------------------
# Build month list (chronologically) from PERIOD_MONTH
# -------------------------
months_available = []
if "PERIOD_MONTH" in df.columns:
...
# fallback to MONTH + YEAR if needed
...
if "YEAR" in df.columns:
    try:
        ...
        # Ensure plain python ints in the list for better widget stability
        all_years = [int(x) for x in yrs.tolist()]
    except Exception:
        all_years = []
if all_years:
...
# -------------------------
# Apply month & year filters globally to 'filtered' df
# -------------------------
filtered = df.copy()
...
# compute available_time robustly (AVAILABLE_TIME_MONTH or AVAILABLE_HOURS)
available_time = None
try:
...
# -------------------------
# Year-to-date (YTD) calculations (unchanged logic)
# -------------------------
ytd_PA = ytd_MA = None
...
# -------------------------
# TOP ROW: KPIs + Donuts (unchanged logic)
# -------------------------
...
with donut1_col:
    ...
    if "CATEGORY" in filtered.columns:
        ...
        if not donut_data.empty:
            ...
            donut_fig = go.Figure(data=[go.Pie(labels=donut_data["CATEGORY"], values=donut_data["DELAY"], hole=0.4,
                                               textinfo="label+percent", hovertemplate="%{label}: %{value:.2f} hrs<extra></extra>")])
            donut_fig.update_layout(margin=dict(t=20, b=20, l=20, r=20))
            # attempt to cache PNG for PDF
    ...
    if "MTN_DELAY_TYPE" in filtered.columns:
        ...
        if not maint_df.empty:
            ...
            if not sched_donut.empty:
                ...
                donut_fig2 = go.Figure(data=[go.Pie(labels=sched_donut["SUB_CATEGORY"], values=sched_donut["DELAY"], hole=0.4,
                                                    textinfo="label+percent", hovertemplate="%{label}: %{value:.2f} hrs<extra></extra>")])
...
# -------------------------
# Trend analysis (Total Delay Hours vs PA%) - slightly adjusted to cache PNG for PDF
# -------------------------
st.subheader("Trend: Total Delay Hours vs PA%")
group_field = granularity

# 52-week cutoff logic (global latest week_start)
if group_field == "WEEK":
...
# group and compute metrics for trend
if group_field == "WEEK":
...
# -------------------------
# CATEGORY filter (below Pareto) -> affects drilldown
# -------------------------
...
# Drill-down table
# -------------------------
# Title includes year filter info
drill_title_suffix = ""
if selected_years:
    drill_title_suffix = f" (Years: {', '.join(str(y) for y in selected_years)})"
st.subheader(f"Drill-down data (filtered by selected category){drill_title_suffix}")
...
# stable sorting: convert WEEK to numeric
...
if "YEAR" in details_out.columns and details_out["YEAR"].notna().any():
    ...
    details_out = details_out.sort_values(by=["YEAR", "WEEK", "START"], ascending=[False, False, True]).reset_index(drop=True)  # descending by YEAR (user preferred descending)
    details_out = details_out.drop(columns=["YEAR"], errors="ignore")
else:
    # descending weeks more helpful: user asked descending is better
...
# AGGRID: make columns auto-size on load based on header/title and make them fixed (not movable)
...
# Add JS hook to auto-size columns on GridReady and prevent moving columns (suppressMovableColumns).
# This requires allow_unsafe_jscode=True in the AgGrid call.
js_auto_size = JsCode("""
function(params) {
  try {
...
""")
grid_options2['onGridReady'] = js_auto_size
# Prevent moving columns
grid_options2['suppressMovableColumns'] = True
...
AgGrid(
    ...
    allow_unsafe_jscode=True,  # required for the auto-size JS above
    height=600,
...
# -------------------------
# Reliability tab: MTTR & MTBF (weekly + monthly)
# Keep it as a separate page/tab - user asked for two pages (Main + Reliability).
# -------------------------
# Prepare weekly/monthly dfs from _mtbf_mttr_res if available
...
# Create tabs: Main Dashboard (we are in it) and Reliability
tabs = st.tabs(["Main Dashboard", "Reliability"])
# Note: All prior content rendered on main automatically (we were writing into main).
# Now build the Reliability tab UI into tabs[1]
with tabs[1]:
    ...
    # Apply global year filter to reliability dfs as well
    ...
    with cols_mttr[0]:
        ...
        if weekly_df.empty:
            ...
            if weekly_df_limited.empty:
                ...
                fig_mttr_w.update_layout(xaxis_title="Week", yaxis_title="MTTR (hours)", showlegend=False, margin=dict(t=60))
                # cache PNG for PDF
    ...
    with cols_mttr[1]:
        ...
        if monthly_df.empty:
            ...
            if "PERIOD_MONTH" in monthly_df_local.columns:
                ...
                fig_mttr_m.update_layout(xaxis_title="Month", yaxis_title="MTTR (hours)", showlegend=False, margin=dict(t=60))
    ...
    with cols_mtbf[0]:
        ...
        if weekly_df.empty:
            ...
            if weekly_df_limited.empty:
                ...
                fig_mtbf_w.update_layout(xaxis_title="Week", yaxis_title="MTBF (hours)", showlegend=False, margin=dict(t=60))
    ...
    with cols_mtbf[1]:
        ...
        if monthly_df.empty:
            ...
            if "PERIOD_MONTH" in monthly_df_local.columns:
                ...
                fig_mtbf_m.update_layout(xaxis_title="Month", yaxis_title="MTBF (hours)", showlegend=False, margin=dict(t=60))
...
# -------------------------
# PDF Export controls (placed near beginning in sidebar)
# Build the KPI text for the PDF header (use values computed above)
# -------------------------
_pdf_kpi_text = ""
try:
...
# Provide Generate & Download buttons
if REPORTLAB_AVAILABLE:
    if st.sidebar.button("Generate PDF (attempt include charts)"):
        # gather PNGs we may have cached; order them (trend, pareto, mttr_w, mttr_m, mtbf_w, mtbf_m)
        pngs = []
        for key in ['pdf_fig_trend', 'pdf_fig_pareto', 'pdf_fig_mttr_w', 'pdf_fig_mttr_m', 'pdf_fig_mtbf_w', 'pdf_fig_mtbf_m']:
            val = st.session_state.get(key)
            if val:
                pngs.append(val)
        pdf_bytes = _create_pdf_bytes("Physical Availability Report", _pdf_kpi_text, pngs)
        if pdf_bytes:
            st.session_state['_last_pdf'] = pdf_bytes
            st.sidebar.success("PDF generated and cached (download available).")
        else:
            st.sidebar.error("PDF generation failed. Check app logs and dependencies (ReportLab + Kaleido).")
    ...
    st.sidebar.info("PDF export unavailable: ReportLab not installed. Install it and ensure Kaleido is available to include charts.")

# -------------------------
# Add helpful debug/info messages about chart export availability
# -------------------------
if st.session_state.get("_last_fig_error"):
    st.sidebar.warning("Last chart -> PNG conversion error (cached): " + str(st.session_state.get("_last_fig_error")))

# If Kaleido/Chrome are missing, provide installation hint (informational)
# Note: instructions depend on environment; print a short hint only.
if REPORTLAB_AVAILABLE and st.sidebar.button("Show chart-export help"):
    st.sidebar.info("If charts are missing in PDF, your environment likely needs Chrome/Chromium for Kaleido.\n"
                    "On many Linux hosts you can install chromium. On Streamlit Cloud you may need to add system packages.\n"
                    "If you can't install Chrome on the host, PDF will contain only KPI summary text.")
